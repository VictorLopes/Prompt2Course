...
def clear_cache():
    """Apaga a pasta de cache se ela existir"""
    if os.path.exists(AUDIO_CACHE_DIR):
        print("Limpando cache de áudio...")
        shutil.rmtree(AUDIO_CACHE_DIR)

async def generate_tts(text, voice, prefix="audio"):
    """Gera o áudio usando um HASH único para evitar colisões"""
...
class PimsleurGenerator:
    def __init__(self, data, native_lang, target_lang, output_filename):
        ...
        self.word_stats = {i: 0 for i in range(len(data[f"target_words_{target_lang}"]))}
    ...
    async def play_and_repeat(self, text, voice, item_type, item_idx, repeats=1, wait_time=2.0):
    ...
    async def build_lesson(self):
        print(f"Iniciando geração do curso: {self.output_filename}")
        ...
        for i in range(len(self.data[self.target_lang])):
            ...
            for w_idx, target_word in enumerate(target_words_en):
                if target_word.lower() in target_text.lower():
                    ...
                    await self.play_and_repeat(target_word, voice, "word", w_idx, repeats=2)
        ...
        while True:
            phrases_done = all(self.phrase_stats[idx] >= TOTAL_REPS_PHRASE for idx in self.introduced_phrases)
            words_done = all(self.word_stats[idx] >= TOTAL_REPS_WORD for idx in self.introduced_words)
        ...
        print(f"Exportando áudio final para '{self.output_filename}'...")
    ...
    async def do_review(self):
        ...
        if item_type == "word":
            ...
            await self.add_narrator(f"Você lembra como se diz a palavra: {native_word}?")
            self.lesson_audio += create_silence(2)
...
async def main():
    if len(sys.argv) < 2:
        print("Erro: Você precisa especificar um arquivo JSON de entrada.")
        print("Uso correto: ./python3/bin/python3 main.py <nome_do_arquivo.json>")
        sys.exit(1)

    json_filepath = sys.argv[1]

    if not os.path.exists(json_filepath):
        print(f"Erro: O arquivo '{json_filepath}' não foi encontrado.")
        sys.exit(1)
    ...
    with open(json_filepath, 'r', encoding='utf-8') as f:
        try:
            ...
            print(f"Erro: O arquivo '{json_filepath}' não é um JSON válido.")
            sys.exit(1)
    ...
    try:
        generator = PimsleurGenerator(curso_data, NATIVE_LANG, TARGET_LANG, output_filename)
